...
@Component({
  selector: 'app-doc-desviodet',
  templateUrl: './doc-desviodet.component.html',
  styleUrls: ['./doc-desviodet.component.css'],
})
export class DocDesviodetComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit(): void {
    ...
    if (sessionStorage.getItem('Item')) {
      ...
      this._accion = this._Item.idDesvio == 0 ? 'N' : 'E';
  ...
  btnRegresar() {
    ...
    this._router.navigate(['/docDesvio']);
  }

  btnFormato() {
    let formatoPDFbase64: string = '';
    this._loading = true;
    this._servicios.wsGeneral('desvio/getPDF', this._Item).subscribe(
      (resp) => {
  ...
  btnEliminar() {
    if (this._Item.idDesvio == 0) this.btnRegresar();

    this._servicios.wsGeneral('desvio/delItem', this._Item).subscribe(
      (resp) => {},
      (error) =>
        this._toastr.error(
          'Error : ' + error.error.ExceptionMessage,
  ...
  btnGuardar() {
    ...
    if (this._Item.idObra == '0') {
      this._toastr.error('Guardar.', 'Falta Administrativo/Obra');
      return;
    }

    if (this._Item.idSupervisor == '0') {
      this._toastr.error('Guardar.', 'Falta Realizado por');
      return;
    }

    if (this._Item.idOperador == '0') {
      this._toastr.error('Guardar.', 'Falta Nombre involucrado');
      return;
    }

    if (this._Item.numTabulador == '0') {
      this._toastr.error('Guardar.', 'Falta numero tabulador');
      return;
    }

    if (this._Item.evento == '0') {
      this._toastr.error('Guardar.', 'Falta Evento');
      return;
    }

    if (this._Item.area == '0') {
      this._toastr.error('Guardar.', 'Falta Area');
      return;
    }
    ...
    let lAccionRecurso: string = 'desvio/insItem';

    if (this._accion == 'E') lAccionRecurso = 'desvio/updItem';
  ...
  setItem() {
    this._Item = {
      ...
      evento: '',
      estado_prisa: false,
      estado_frustacion: false,
      estado_fatiga: false,
      estado_complacencia: false,
      error_ojos_no_tarea: false,
      error_mente_no_tarea: false,
      error_mala_colocacion: false,
      error_perdida_equilibrio: false,
      observaciones: '',
      caidas: false,
      golpes: false,
      quemaduras: false,
      asfixia: false,
      eletrocucion: false,
      objetos_que_golpean: false,
      accidentes_vehiculares: false,
      actitud: false,
      procedimiento_de_trabajo: false,
      permiso_de_trabajo: false,
      e_p_p: false,
      herremientas_y_equipos: false,
      posicion_de_las_personas: false,
      orden_y_limpieza: false,
      reaccion_de_la_persona: false,
      compromiso_establecido: '',
      nomSupervisor: '',
      nomOperador: '',
      nomObra: '',
    };
